...
5. 默认情况下，LuaNilGuard 会把 `id.lua` 和 `*_id.lua` 从扫描与宏缓存解析中直接跳过，避免超大生成文件拖慢体验。如果你希望从特定文件提取编译期事实，请在 `config/preprocessor_files.json` 里显式配置，再查看工具能提取哪些事实：
...
被显式配置为预处理输入的文件会提供编译期非 nil 事实，但不会被当作普通业务 Lua 文件参与候选扫描。
LuaNilGuard 会为这些预处理文件编译并复用本地宏缓存，这样重复执行时不需要每次重新解析未变更的超大宏字典文件。
...
这些模式已经包含在默认生成的 `sink_rules.json` 中，也是本工具在首次试用时最希望让客户直接感知到的核心价值。`member_access.receiver` 默认不启用；如果你的仓库确实需要大范围 receiver nil 检查，请手动添加该规则。
...
如果你的仓库里有超大的“宏定义/默认值”文件（例如每行都是 `NAME = ""` 或 `Defaults.Name = 0`，并在编译阶段做替换），请把需要解析的文件加入 `config/preprocessor_files.json`，然后运行：
...
`sink_rules.json` 默认不会包含 `member_access.receiver`。如果你希望启用 receiver 检查，可显式加入：
...
`domain_knowledge.json` 用于定义“零 AST 快速裁剪”规则。默认模板已包含你提到的 `_name_*`、`_cmd_*` 和全大写下划线宏模式，可直接把这些变量视为已知非 nil，从而跳过不必要的候选：
...
1. 先保留 `lua-nil-guard init-config` 生成的默认配置。
2. 只有在确实需要 receiver 全覆盖时才手动添加 `member_access.receiver`。
3. 先加“命名约定很明确”的 domain 规则，再逐步放宽。

示例：把 `_id_.*` 视为稳定的非 nil 数字变量：
...
- 优先使用精确前缀（`^_name_`、`^_cmd_`、`^_id_`），不要一开始就用宽泛正则。
- 先限定 `applies_to_sinks`，再考虑扩大范围。
- 先用 `report-file` 在单文件验证，再扩到全仓扫描。

`preprocessor_files.json` 用于控制“超大生成 Lua 文件”的分类方式：

- `preprocessor_files` / `preprocessor_globs`：按宏字典解析（仅提取编译期事实，不参与常规候选扫描）
- `skip_review_files` / `skip_review_globs`：完全跳过（不扫描、不解析宏、不参与缓存）
...
如果你确实希望把某些 `id.lua` 风格文件当作宏字典解析，可以把规则移动到 `preprocessor_*`，并从 `skip_review_*` 中移除。

LuaNilGuard 可提取的有界编译期事实例如：

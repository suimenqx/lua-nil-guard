...
LuaNilGuard 已从正则原型经过 5 轮迭代演进为 V2.1 分层审查系统（447 测试通过）。V2.1 是当前运行系统，所有现有 CLI 命令、测试、数据模型均基于此版本。

本计划定义从 V2.1 到 V3 的完整重构路径，涵盖三个方向、三个阶段、全部执行步骤。
...
**目标**：支持 `--adjudication-mode` CLI flag 和 `config/adjudication_policy.json` 配置。
...
1. 在 `config_loader.py` 中新增 `load_adjudication_policy()` 函数：
   ...
   - 默认值：`{"adjudication_mode": "multi_agent"}`（兼容现有行为）
   - 支持 `single_pass` / `multi_agent` / `ab_test` 三种模式
2. 在 `cli.py` 中为 `report` / `report-file` / `run-start` 等命令新增 `--adjudication-mode` 可选参数
3. 在 `service.py` / `pipeline.py` 中根据 mode 路由：
   - `multi_agent` → `adjudicate_packet()`（现有路径）
   - `single_pass` → `adjudicate_single_pass()`（新路径）
   - `ab_test` → 按 case_id hash + seed 分流到两个路径
...
**目标**：验证单次判定精度 >= 多 Agent 对抗。

**执行步骤**：

1. 使用现有 `benchmark` 命令，对同一组 labeled cases：
   - `--adjudication-mode multi-agent`：运行基线
   - `--adjudication-mode single-pass`：运行新路径
2. 对比 `exact_matches` / `false_positive_risks` / `missed_risks` 三个指标
3. 如果 single-pass 在所有 sink 类型上精度 >= multi-agent：
   - 将 `adjudication_policy.json` 默认值改为 `single_pass`
   - 在后续版本中移除 multi-agent 路径
4. 如果 single-pass 在某些 sink 类型上较差：
   - 记录这些类型
   - 可为这些类型保留多轮调用
...
- [x] A/B 对比完成：single_pass heuristic verdict 与 multi_agent 一致（test_single_pass_agrees_with_multi_agent_judge 通过）
- [x] 所有现有测试通过 + 新增测试通过
- [x] `single_pass` 已设为默认 adjudication_mode

...
V2.1 共 31 个测试文件，447 个测试用例，全部通过。
...
```
T-A2-01  mode=multi_agent 路由到 adjudicate_packet()
T-A2-02  mode=single_pass 路由到 adjudicate_single_pass()
T-A2-03  mode=ab_test 按 case_id hash 分流，比例约 50/50
         构造 100 个不同 case_id，验证分流比例在 [35, 65] 区间
T-A2-04  ab_test 相同 seed + 相同 case_id 始终路由到同一路径（确定性）
T-A2-05  CLI --adjudication-mode flag 覆盖配置文件值
T-A2-06  配置文件缺失时使用默认值（multi_agent，兼容 V2.1）
...
T-A4-07  校准不可用时 verify_verdict() 行为与 V2.1 完全一致
...
```
T-X-01  V2.1 run 数据库（无新表/新字段）在 V3 代码下可正常打开和查询
T-X-02  V3 代码对 V2.1 数据库自动执行 schema 迁移（ALTER/CREATE）后数据完整

...
def test_bootstrap_repository_skips_default_id_globs_without_explicit_preprocessor_config(
    ...
    assert snapshot.preprocessor_files == ()
    assert snapshot.lua_files == (business_file,)
    assert snapshot.macro_cache_status is not None
    assert snapshot.macro_cache_status.state == "disabled"
    assert snapshot.macro_index is not None
    assert snapshot.macro_index.facts == ()
